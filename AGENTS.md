...
- **Record the demo while the extension is working, not before.** Capture it after the browser verification has passed, in the same live session, with the relay up and the extension badging. Scroll to a card that is carrying a `LIKELY AI SLOP` badge and hold on it long enough to read the number, then show a `CHECK THIS` outline and a card left untouched. Give it at least eight seconds.
- **A demo with no badge in it is not a product demo.** Before you call it done, open the clip's own frames and find them: at least one frame showing the badge with its score, and at least one showing the outline. Write those frame numbers into `LAUNCH.md`. Footage of the real feed with nothing marked on it proves the browser works, not that the product works, so if no frame carries a badge the clip is rejected and recorded again. Do not put it on the landing page until it passes this check.
...
- **GitHub**: `https://github.com/jxspam/slop-detector`, already authenticated through `gh`. Add it as the `origin` remote, push the branch `v5-demo-gate`, and open the PR from that branch. Never push to `main`, and never touch an existing branch or an existing pull request.
