...
function parseEvaluationResponse(data, criteriaLength = CRITERIA_LEVELS.length) {
  ...
  if (typeof answer.score !== "number" || Number.isNaN(answer.score)) {
...
function determineBand(score, thresholds = {}) {
  ...
  if (score >= high) {
    return {
      ...
      label: "AI SLOP",
      score,
      color: thresholds.colorHigh || "#FF3B30",
      borderWidth: "3px"
    };
  }
  ...
  if (score >= check) {
    return {
      ...
      label: "CHECK THIS",
      score,
      color: thresholds.colorCheck || "#FF9500",
      borderWidth: "3px"
    };
  }
  ...
  return {
    ...
    label: "",
    score,
    color: thresholds.colorClean || "#34C759",
    borderWidth: "2px"
  };
}
...
async function judgeCard(text, apiKey, fetchFn = globalThis.fetch, options = {}) {
  ...
  const parsed = parseEvaluationResponse(data, (options.criteria || CRITERIA_LEVELS).length);
  ...
  return {
    ...
    borderWidth: bandInfo.borderWidth,
    model: parsed.model,
    latencyMs
  };
}
